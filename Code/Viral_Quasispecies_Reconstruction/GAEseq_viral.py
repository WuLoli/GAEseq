...
MEC_record = {}
MEC_threshold = float(data[16][1])

while rank_record[1] - rank_record[0] != 1:
	for ploidy_candidate in [ploidy, ploidy + 1]:
		if ploidy_candidate not in MEC_record:
			# dimension of each layer
			num_layers = int(data[10][1])
			C_i = [int(len_haplo - i / (num_layers + 1) * (len_haplo - ploidy_candidate)) for i in range(1, num_layers + 1)]
			...
			V = tf.placeholder(tf.float32, [ploidy_candidate, len_haplo * 4], name = 'V') # placeholder for haplotype matrix in unfoldered tensor structure
			...
			W_dense = tf.get_variable('W_dense', shape = [C_i[-1], ploidy_candidate], initializer = tf.contrib.layers.xavier_initializer())
			...
			MEC_experiment = []
			haplo_experiment = np.zeros((ploidy_candidate, len_haplo, num_exp))
			flag = 10 ** 8
			time_record = []
			...
			for iteration in range(num_exp):
				...
				with tf.Session(config = setting) as sess:
					...
					epoch_haplo = np.zeros((ploidy_candidate, len_haplo, training_epoch))
					epoch_MEC = []
				...
				# # record MEC results
				# with open(zone_name + '_MEC_result_{}_Strains.txt'.format(ploidy_candidate), 'a') as f:
				# 	f.write('Experiment' + str(iteration + 1) + ' : ' + str(MEC_experiment[-1])  + '\n')

				end_time = time.time()
				time_record.append(end_time - start_time)
				print('Checking Rank: {} - '.format(ploidy_candidate) + 'Experiment: {}/{} - ETA: {}s '.format(iteration + 1, num_exp, int(np.mean(time_record) * (num_exp - (iteration + 1)))), end = '\r')
			
			result = haplo_experiment[:, :, np.argmin(np.array(MEC_experiment))]
			MEC_record[ploidy_candidate] = MEC(SNVmatrix, result)
			print('\nThe lowest MEC of all experiments with {} strains is : {}'.format(ploidy_candidate, MEC_record[ploidy_candidate]))

			# estimate frequencies
			index = []
			for i in range(SNVmatrix.shape[0]):
				dis = np.zeros((result.shape[0]))
			...
			continue
	CR = 1 - MEC_record[ploidy_candidate] / MEC_record[ploidy_candidate - 1]
	if CR <= MEC_threshold:
		rank_record[1] = ploidy_candidate - 1
		ploidy = (rank_record[0] + rank_record[1]) // 2
	else:
		rank_record[0] = ploidy_candidate - 1
		if rank_record[1] == 0:
			ploidy *= 2
		else:
			ploidy = (rank_record[0] + rank_record[1]) // 2 
